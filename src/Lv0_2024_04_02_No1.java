...
public class Lv0_2024_04_02_No1 {
    public int solution(int[] rank, boolean[] attendance) {
        int answer = 0;
        int n = rank.length;
        Integer[] re= Arrays.stream(rank).boxed().toArray(Integer[]::new);
        Arrays.sort(re, Collections.reverseOrder());
        int[] top3 = new int[3];
        int tindex =0;
        for(int i = 0; i < n; i++){
            if(attendance[i] && tindex<=3){
        ...
        answer =  10000 * rank[n-1] + 100 * rank[n-2]+rank[n-3];
        return answer;
    }
    public static void main(String[] args) {
        Lv0_2024_04_02_No1 s = new Lv0_2024_04_02_No1();
